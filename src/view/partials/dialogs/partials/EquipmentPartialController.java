			...
			row.hoverProperty().addListener(
					(observable) -> {
						...
						if (row.isHover() && weapon != null) {
							...
							weaponView.getDialogStage().setX(row.getLayoutX());
							weaponView.getDialogStage().setY(
	...
	void readItems() {
		...
		try {
			...
			Dialogs.create().masthead("Read Error")
					.masthead("Couldn't read the one of the Equipment Files properly")
