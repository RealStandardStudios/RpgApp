...
public class EquipmentPartialController extends NewCharacterPartialController {
	...
	tableWeaponsAvailable.getSelectionModel().selectedItemProperty().addListener((observable, oldValue, newValue) -> handleSelectedAvailableItem(newValue));
	tableArmorAvailable.getSelectionModel().selectedItemProperty().addListener((observable, oldValue, newValue) -> handleSelectedAvailableItem(newValue));
	tableGoodsAvailable.getSelectionModel().selectedItemProperty().addListener((observable, oldValue, newValue) -> handleSelectedAvailableItem(newValue));
	...
	try {
	    ...
	    Dialogs.create().masthead("Read Error").masthead("Couldn't read the one of the Equipment Files properly").message(e.getMessage()).showWarning();
	    e.printStackTrace();
	}
    }
	...
	// getCharacter().setInventory(new Inventory(obsChosenWeapons,
	// obsChosenArmor, obsChosenGoods, goldRemaining));
    }
