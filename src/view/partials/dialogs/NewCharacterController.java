...
public class NewCharacterController extends DialogController {
     ...
     * @param ui the root UI for the program
    ...
    private void handleTabChanged(Event event) {
        ...
        switch (tabChanged.getId()) {
            case "tpAbilityScores":
                ...
                if (!aeAdded) {
                    getCharacter().getEffects().addAll(Arrays.asList(getCharacter().getRace().getRacialModifiers()));
                    aeAdded = true;
                }
                if (!teAdded) {
                    getCharacter().getRace().getRacialTraits().stream().forEach((t) -> {
...
//				e.printStackTrace();
                Dialogs.create().title("ERROR").masthead("There was an Error saving the Character").message(Arrays.toString(e.getStackTrace())).showWarning();
            }
        }
    }
     ...
     * gets the Partials
     *
     * @return the partials for the controller
     */
